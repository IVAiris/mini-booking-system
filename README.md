...
Учебный сервис онлайн-записи на слоты. Текущий модуль — **auth-lite**: регистрация, вход по email/паролю, серверные сессии и минимальный контроль доступа по ролям (`user`/`admin`/`super_admin`). Домены слотов, бронирований и уведомлений в этот модуль не входят — они реализуются на последующих шагах.
